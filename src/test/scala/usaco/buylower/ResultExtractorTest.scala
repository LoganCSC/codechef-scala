...
class ResultExtractorTest extends FunSuite {
  ...
  test("2 element increasing") {
    assertResult("1 2") {
      new ResultExtractor(List(List(List(2, 1)))).getResult
    }
  }

  test("2 element decreasing") {
    assertResult("2 1") {
      new ResultExtractor(List(List(List(1), List(2)))).getResult
    }
  }

  test("example from problem") {
    assertResult("2 4") {
      new ResultExtractor(List(List(List(4, 3), List(5, 6)))).getResult
    }
  }

  test("my tough case") {
    assertResult("3 39") {
      new ResultExtractor(List(
